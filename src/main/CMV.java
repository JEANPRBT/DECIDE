...
public final class CMV {
    ...
    */
    ...
    private void setLIC8() {
        ...
        for (int i = 0; i < numPoints - this.parameters.a_pts - this.parameters.b_pts - 2; i++) {
            Point2D p1 = points[i], p2 = points[i + this.parameters.a_pts + 1], p3 = points[i +
                    this.parameters.a_pts + this.parameters.b_pts + 2];
    ...
    private void setLIC9(){
        ...
        for(int i = 0; i < numPoints - (parameters.c_pts + 1) - (parameters.d_pts + 1); i++){
            ...
            Point2D p1 = this.points[i],
                    p2 = this.points[i + parameters.c_pts + 1],
                    p3 = this.points[i + parameters.c_pts + 1 + parameters.d_pts + 1];
            Point2D p1_p2 = new Point2D.Double(p2.getX() - p1.getX(), p2.getY() - p1.getY());
            Point2D p2_p3 = new Point2D.Double(p2.getX() - p3.getX(), p2.getY() - p3.getY());

            if((p1.getX() == p2.getX() && p1.getY() == p2.getY()) || (p3.getX() == p2.getX() && p3.getY() == p2.getY()))
                continue;

            double magp1p2 = Math.sqrt((Math.pow(p1_p2.getX(), 2) + Math.pow(p1_p2.getY(), 2)));
            double magp2p3 = Math.sqrt((Math.pow(p2_p3.getX(), 2) + Math.pow(p2_p3.getY(), 2)));
            double dotProduct = p1_p2.getX() * p2_p3.getX() + p1_p2.getY() * p2_p3.getY();
    ...
    */
     ...
     * "There exists at least one set of two data points, separated by exactly K PTS consecutive intervening points,
     * which are a distance greater than the length, LENGTH1, apart. In addition, there exists at least one set of
     * two data points (which can be the same or different from the two data points just mentioned), separated by
     * exactly K PTS consecutive intervening points, that are a distance less than the length, LENGTH2, apart.
     * Both parts must be true for the LIC to be true. The condition is not met when NUMPOINTS < 3."
     * 0 ≤ LENGTH2
     */
    private void setLIC12(){
     ...
     * A_PTS and B_PTS consecutive intervening points, respectively, that cannot be contained within or
    ...
    private boolean circleContain(Point2D p1, Point2D p2,Point2D p3){
        ...
        if (crossProduct == 0) {
            if (d12 + d23 <= 2 * this.parameters.radius1 ||
                    d23 + d13 <= 2 * this.parameters.radius1 ||
                    d12 + d13 <= 2 * this.parameters.radius1)
                return true;
        }
        if (d12 == 0 && d13 == 0 && d23 == 0) return true;

        double r = (d12 * d23  * d13) / (Math.sqrt((d12 + d13 + d23) * (d12 + d13 - d23) *
                (d13+d23 - d13) * (d12 + d23 - d13)));

        return r <= this.parameters.radius1;
    }
    ...
    private boolean pointsInCircle(Point2D pointA, Point2D pointB,  Point2D pointC, double radius) {
        ...
        // Collinear points cannot form a circumcircle with finite radius
        if (l_a + l_b == l_c || l_a + l_c == l_b || l_b + l_c == l_a) return false;
